  ...
  return (
    <Box sx={{minWidth:'200px',width:{md:'20%',xs:'100%'},height:{md:'300px',sm:'170px',xs:'260px'},display:'flex',flexDirection:{md:'column',sm:'row',xs:'column'},alignItems:'center',justifyContent:{md:'center',xs:'space-between'},background:`${theme.colors.alpha.white[100]}`,boxShadow:`${theme.colors.shadows.card}`,borderRadius:'5px',marginBottom:{md:'0',xs:'20px'}}}>
        ...
        <Box sx={{display:{md:'none',xs:'block'},width:'100%',fontSize:'17px',marginBottom:'10px',paddingLeft:'20px',paddingTop:'5px',fontWeight:500,height:'40px',background:`${theme.colors.alpha.black[10]}`}}>Default Address</Box>
            ...
            {user.currentUser?.data?.address.length !==0 ?<Typography><span style={{fontSize:'16px',fontWeight:500}}>Address :</span> {user.currentUser?.data?.address[0]?.locality}, {user.currentUser?.data?.address[0]?.city}, {user.currentUser?.data?.address[0]?.state} - {user.currentUser?.data?.address[0]?.zipcode}</Typography>:null}

            {user.currentUser?.data?.address.length ===0 ?<Typography><span style={{fontSize:'16px',fontWeight:500}}>Address :</span> Not Available</Typography>:null}
            
            {user.currentUser?.data?.phone !==undefined ? <Typography><span style={{fontSize:'16px',fontWeight:500}}>phone :</span> {user.currentUser.data.phone}</Typography>:null}
        </Box>
    </Box>
  )
}
