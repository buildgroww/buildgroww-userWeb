import { Box, Button, Dialog, DialogActions, DialogContent, DialogContentText, DialogTitle, FormControl, IconButton, InputLabel, Menu, MenuItem, Select, styled, TextField, Typography, useTheme, Zoom } from '@mui/material'
...
function Block2({item,index}) {
  ...
  return (
    ...
    :<Box sx={{display:'flex',alignItems:'center',padding:'20px',position:'relative',width:'100%',minHeight:'70px',border:`1px solid ${theme.colors.alpha.black[30]}`}}>
        <Box>
            <span style={{fontWeight:500}}>{index+1}. Address : </span>
            <span style={{fontSize:'16px'}}>{item?.locality}, {item?.city}, {item?.state} - {item?.zipcode}, {item?.country}</span>
        </Box>
        <Box>
            <IconButton sx={{position:'absolute',right:0,top:3,cursor:{md:'pointer',xs:'none'}}} onClick={handleClick}>
            ...
            >
                <MenuItem sx={{display:'flex',alignItems:'center',gap:'5px'}} onClick={(e)=>handleEdit(e,index)}><EditOutlinedIcon sx={{fontSize:'20px'}}/>edit</MenuItem>
                <MenuItem sx={{display:'flex',alignItems:'center',gap:'5px'}} onClick={(e)=>handleDelete(e,index)}><DeleteIcon sx={{fontSize:'20px'}}/>delete</MenuItem>
            </Menu>
        </Box>
    </Box>}
    <div>
