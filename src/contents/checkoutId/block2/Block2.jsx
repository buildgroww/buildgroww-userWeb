...
import Block from './addAddress/Block';
import BlockEdit from './editAddress/BlockEdit';
...
export default function Block2(props) {
    ...
    const [addAddress, setAddAddress] = useState('none');
    ...
    const handleDeliver = ()=>{
        ...
        setShow('none');
        setHide('block');
        props.setAddressIndex(defaultValue);
    }
    const handleChange = ()=>{
    ...
    return (
      ...
      >
        {user.currentUser?.data?.address.map((item,index)=>(
            <BlockEdit key={index} item={item} index={index} setDefaultValue={setDefaultValue} />
        ))}
        {user.currentUser?.data?.address.length!==0 ? <Box sx={{display:'flex',justifyContent:'flex-end'}}>
    ...
    <Button onClick={()=>{setAddAddress('flex')}} sx={{width:'100%',borderRadius:'1px',justifyContent:'flex-start',cursor:{md:'pointer',xs:'none'},marginTop:'10px',height:'50px',display:`${show}`,gap:'20px',alignItems:'center',padding:'0 20px',
          background:`${theme.colors.alpha.white[100]}`,border:`1px solid ${theme.colors.alpha.black[10]}`,
          boxShadow:`${theme.colors.shadows.cardSm}`,color:`${theme.colors.primary.main}`}}>
            ...
            {user.currentUser?.data?.address.length!==0 ?<Typography sx={{fontSize:'16px'}}>Add a new address</Typography>:<Typography sx={{fontSize:'16px'}}>Add address to Continue</Typography>}
    </Button>
    <Block show={addAddress} setShow={setAddAddress} />
    </div>
    )
}
