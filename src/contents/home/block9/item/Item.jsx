...
export default function Item({posterLinks}) {
  ...
  return (
    <ImageContainer onClick={handleClick} sx={{position:'relative', margin:'5px 10px', borderRadius:'5px'}}>
        <img style={{width:'100%', height:'100%'}} src={posterLinks.url} alt="card"/>
        ...
        {/* <Box sx={{position:'absolute', top:{md:'78%',xs:'75%'} , paddingLeft:'26px', left: 0,right:0 ,width:'100%'}}>
            <Typography sx={{fontSize:{md:'16px',sm:'14px',xs:'13px'}, fontWeight: 500, }}>{posterLinks.name}</Typography>
            <Typography sx={{fontSize:'14px', fontWeight: 500,}}>{posterLinks.Price}</Typography>
        </Box>    */}
    </ImageContainer>
  )
}
