...
const Block4 = ({packages,pack,setPack}) => {
  ...
  const result = packages && packages.data?.data?.filter(check)
    ...
    function check(item){
      return item.type===`${pack.type}` && item.plan===`${pack.plan}`
    }
  ...
  useEffect(() => {
    let glider = document.getElementById('packageGlider4')

    if(glider){
      new Glider(document.getElementById('packageGlider4'), {
        // Optional parameters
        slidesToShow: 1,
        slidesToScroll: 1,
        draggable: true,
        ...
        dots: ".pagination",
        arrows: {
          prev: ".packageGlider-prev4",
          next: ".packageGlider-next4"
        },
  ...
  return (
   ...
   <Box className='packageGlider-inner4' id='packageGlider4' sx={{margin:{sm:'0 100px',xs:'0 10px'}}}
   ...
   <span role="tablist" className="pagination"></span>
        <span className="packageGlider-prev4"><ChevronLeftIcon/></span>
        <span className="packageGlider-next4"><ChevronRightIcon/></span>
    </div>
  )
}
