...
function Block1({packages,pack,setPack}){
  // console.log(packages)
    ...
    function check(item){
      return item.type.toLowerCase()==="basic"
    }
    ...
    useEffect(() => {
      let glider = document.getElementById('productsGlider1')

      if(glider){
        new Glider(document.getElementById('productsGlider1'), {
          // Optional parameters
          slidesToShow: 1,
          slidesToScroll: 1,
          draggable: true,
          ...
          dots: ".pagination",
          arrows: {
            prev: ".productsGlider-prev1",
            next: ".productsGlider-next1"
          },
  ...
  <Container  className='productsGlider-inner1' id='productsGlider1'>
    {result && result.map((item,index)=>(
      ...
      <CardContent sx={{padding:`${but}`==="all"?"0px 10px 30px 20px":"0px 10px 0px 20px",marginTop:'-33px', display:"flex", justifyContent:'center', alignItems:"space-between", flexDirection:"column"}}>
       ...
       <Box sx={{width:"120px", height:"30px", background:"red", color:"white", borderRadius:"15px", display:"flex", justifyContent:"center", alignItems:"center", fontSize:"20px", fontWeight:"500"}}>Save 80%</Box>
        </Box>
...
{item.technicalDetails.dns ? 
  ...
  <span role="tablist" className="pagination"></span>
        <span className="productsGlider-prev1"><ChevronLeftIcon/></span>
        <span className="productsGlider-next1"><ChevronRightIcon/></span>
          </div>
  )
}
