...
const Block2 = ({packages,pack,setPack}) => {
  // console.log(packages)
    ...
    function check(item){
      return item.type.toLowerCase()==="advance"
    }
    ...
    useEffect(() => {
      let glider = document.getElementById('productsGlider2')

      if(glider){
        new Glider(document.getElementById('productsGlider2'), {
          // Optional parameters
          slidesToShow: 1,
          slidesToScroll: 1,
          draggable: true,
          ...
          dots: ".pagination",
          arrows: {
            prev: ".productsGlider-prev2",
            next: ".productsGlider-next2"
          },
  ...
  return (
  <>
  <section className="section1 glider-section1">
    <div className="container1 glider-column1" style={{position:'relative',width:'100%'}}>
  ...
  <Container sx={{">div":{gap:'10px'}}} className='productsGlider-inner2' id='productsGlider2'>
    {result && result.map((item,index)=>(
      ...
      <CardContent sx={{padding:`${but}`==="all"?"0px 10px 30px 20px":"0px 10px 0px 20px",marginTop:'-33px', display:"flex", justifyContent:'center', alignItems:"space-between", flexDirection:"column"}}>
        <Paragraph variant="h4" color="teal" sx={{fontSize:{sm:"24px",xs:'16px'},marginBottom:"5px",textAlign:"center", letterSpacing:"1px" }}>
       ...
       <Box sx={{width:"120px", height:"30px", background:"#DA4453", color:"white", borderRadius:"15px", display:"flex", justifyContent:"center", alignItems:"center", fontSize:"20px", fontWeight:"500"}}>Save 80%</Box>
        </Box>
...
{item.technicalDetails.dns ? 
  ...
  <span role="tablist" className="pagination"></span>
        <span className="productsGlider-prev2"><ChevronLeftIcon/></span>
        <span className="productsGlider-next2"><ChevronRightIcon/></span>
          </div>
          </section>
          </>
  )
