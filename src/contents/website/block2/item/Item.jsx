...
export default function Item({posterLinks}) {
  ...
  return (
    <SliderContainer>
        <ImageContainer onClick={()=>handleClick(posterLinks)} sx={{borderRadius:'8px'}}>
            <Image src={posterLinks && posterLinks?.productImages[0]?.path} />
            <Box sx={{width:'100%',height:{md:'20%',sm:'25%',xs:'30%'},display:'flex', justifyContent:'space-between', alignItems:'center',padding:{md:'0 25px',sm:'0 25px',xs:'0 15px'}}}>
                <Box>
                    <Head>{posterLinks && posterLinks.subCategory && posterLinks.subCategory}</Head>
                    <Para>Rs {posterLinks && posterLinks?.price?.cost}/-</Para>
                </Box>
